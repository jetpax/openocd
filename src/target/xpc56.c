...
// WARNING: ACCESS_AUX_TAP_xxx values may differ depending on xPC56 subfamily
...
static int xpc56_nexus_mem(struct target *target, target_addr_t address, uint32_t size, uint32_t count, uint8_t *buffer, bool write)
{
	...
	for (uint32_t i = 0; i < count; i++) {
		...
		if (!write) {
			switch (size) {
			case 1: val = buffer[0]; break;
			case 2: val = be_to_h_u16(buffer); break;
			case 4: val = be_to_h_u32(buffer); break;
			}
		}
...
static uint32_t xpc56_cpuscr_exec(struct target *target, uint32_t inst, bool ffra, uint32_t *wbbrl)
{
	...
	xpc56_e200_ir_rw(target, E200_IR_GO | E200_IR_NOREG); // E200_IR_CPUSCR not working
	...
	if (scan[5] & BIT(13)) //test PCINV in CTL
		return ERROR_FAIL;
	else
		return ERROR_OK;
}
...
{
	assert(size == 1 || size == 2 ||size == 4);
	// We are retrieving and storing memory data according to the access size: so copying from target to local using 32 bits access
	// will reverse bytes in memory, but each CPU reading its own memory via 32 bits access will provide same values.
	...
	for (uint32_t i = 0; i < count; i++) {
		...
		if (xpc56_cpuscr_exec(target, inst[size], true, &data))
			return ERROR_FAIL;
		switch (size) {
		case 1: *buffer = (uint8_t) data; break;
		case 2: *(uint16_t*) buffer = (uint16_t) data; break;
		case 4: *(uint32_t*) buffer = data; break;
		}
		address += size;
		buffer += size;
	}
...
static int xpc56_cpuscr_memw(struct target *target, target_addr_t address, uint32_t size, uint32_t count, uint8_t *buffer)
{
	assert(size == 1 || size == 2 ||size == 4);
	...
	for (uint32_t i = 0; i < count; i++) {
		xpc56_cpuscr_reg(target, 1, true, *(uint32_t*)buffer);	// reading past the buffer but dont care
		buffer += size;
