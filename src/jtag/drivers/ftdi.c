...
static void ftdi_execute_command(struct jtag_command *cmd)
{
	switch (cmd->type) {
		case JTAG_RESET:
			fprintf(stderr, "FTDI RESET --- t:%x   s:%x\n", cmd->cmd.reset->trst, cmd->cmd.reset->srst);
			ftdi_execute_reset(cmd);
			break;
		case JTAG_RUNTEST:
			fprintf(stderr, "FTDI RUNTEST\n");
			ftdi_execute_runtest(cmd);
			break;
		case JTAG_TLR_RESET:
			fprintf(stderr, "FTDI TLR ---\n");
			ftdi_execute_statemove(cmd);
			break;
		case JTAG_PATHMOVE:
			//fprintf(stderr, "FTDI PM\n");
			ftdi_execute_pathmove(cmd);
			break;
		case JTAG_SCAN:
			//fprintf(stderr, "FTDI SCAN\n");
			ftdi_execute_scan(cmd);
			break;
		case JTAG_SLEEP:
			fprintf(stderr, "FTDI SLP\n");
			ftdi_execute_sleep(cmd);
			break;
		case JTAG_STABLECLOCKS:
			fprintf(stderr, "FTDI SCLK\n");
			ftdi_execute_stableclocks(cmd);
			break;
		case JTAG_TMS:
			fprintf(stderr, "FTDI TMS\n");
